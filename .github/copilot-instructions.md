...
- Kernels are two-tier. The GENERAL contract on `_TransitionKernel` — `_branch_sum(f, ker, s, x, args...)`, `_foreach_branch(f, ker, s, x, args...)` (top-level `f` with explicit payload `args`, never a capturing closure), `_draw_branch_index` — plus `_forces_brent` defaulting to `true`. A new kernel type (e.g. wrapping a POMDPs transition) implements the general contract only and automatically gets the Brent solve paths through the generic `_expected_value` default. The STRUCTURED `_QuadratureKernel` additionally provides indexed access — `_branch_weights(ker, s, x)` (weight container, fetched once per `(s, x)`) and `_branch_state(ker, s, x, j)` (next state on branch `j`; derivative-based consumers re-evaluate it at perturbed actions holding `j` fixed) — on which the FOC paths rely.
