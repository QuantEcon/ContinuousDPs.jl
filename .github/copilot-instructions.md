...
- `ContinuousDP` stores an `actions::ActionSpace`: `ContinuousActions(x_lb, x_ub)` (one-dimensional box; the legacy positional `x_lb, x_ub` constructor arguments wrap into this), `ContinuousActions{M}(x_lb, x_ub)` (M-dimensional box with tuple-valued bounds; actions passed to `f`/`g` as length-M indexable collections; policies stored as `n x M` matrices), or `DiscreteActions(vals)` (finite set of action values of any homogeneous type).
