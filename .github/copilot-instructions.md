...
- Kernels are two-tier. The GENERAL contract on `_TransitionKernel` — `_branch_sum(f, ker, s, x, args...)` and `_foreach_branch(f, ker, s, x, args...)` (top-level `f` with explicit payload `args`, never a capturing closure), plus `_draw_next_state(rng, ker, s, x)` (required only when `simulate!` support is desired) and `_forces_brent` defaulting to `true`. A new kernel type (e.g. wrapping a POMDPs transition) implements the general contract and gets the Brent solve paths through the generic `_expected_value` default and policy evaluation through the traversal-based assembly; `_draw_branch_index` remains a structured-tier helper (general kernels need not expose stable branch indices). The STRUCTURED `_QuadratureKernel` additionally provides indexed access — `_branch_weights(ker, s, x)` (weight container, fetched once per `(s, x)`) and `_branch_state(ker, s, x, j)` (next state on branch `j`; derivative-based consumers re-evaluate it at perturbed actions holding `j` fixed) — on which the FOC paths rely. Overriding `_forces_brent(ker) = false` opts into the FOC solvers and requires the indexed access with stable branch count/identity under action perturbations and action-independent probabilities.
- Performance pattern (measured, do not "simplify" away): the traversal indirection is NOT free (~50-80 B per call survives even the payload pattern), so hot structured paths use direct indexed loops as specializations — `_expected_value(::_QuadratureKernel, ...)` overrides the generic default, and the `_policy_system_lu` per-state assembly dispatches on the kernel tier (`_sub_state_rows!`/`_append_state_rows!`: indexed loops for the structured kernel, traversal-based for general kernels). Byte-identity of the structured paths against the pre-kernel code is the enforced contract. Do not write raw `for j in eachindex(cdp.weights)` expectation loops in new code — go through the kernel primitives.
- Kernel injection: a `ContinuousDP` may carry a ready-made `_TransitionKernel` in its `weights` slot (internal representation; `_build_kernel`/`_process_weights` pass it through, nothing is probed) — the entry point for non-quadrature kernels such as the POMDPs adapter's. General kernels used with `simulate!` must implement `_draw_next_state(rng, ker, s, x)`.
- Construction: sweep-level entry points (sweeps, `s_wise_max!`, `evaluate_policy!`/`_policy_system_lu`, `evaluate!`, `simulate!`, `PolicyFunction`) call `_build_kernel(cp)` once per call; the per-state Tier-A solvers (`_s_wise_max!`, `_objective`, `_objective_and_deriv`, `_s_wise_max_foc!`, `_negH_multi!`, `_s_wise_max_multi!`, `_s_wise_max_discrete!`) receive `ker` as their second argument. For fixed weight vectors `_build_kernel` is free (no user function is evaluated); for callable weights it detects the arity by `hasmethod` (`weights(s, x)` wins if both apply) but never evaluates the callable — construction sits on the sweep path, so the one probe call validating the returned container happens once per solve instead (`_validate_weights` at `CDPWorkspace` creation). The probe point (first node, first/lower-bound action) need not be feasible for the model, so if the probe call throws, validation is skipped rather than rejecting a valid problem.
- `ContinuousDP.weights` may be a `Vector{Float64}` or a callable `weights(s)` / `weights(s, x)` (hybrid contract: `Tuple`/`SVector` returns keep the sweeps allocation-free, `Vector` returns are allocation-lean; the solve operators do NOT validate that weights sum to 1 — sub-stochastic kernels are permitted and act as extra discounting; `simulate!` by contrast REQUIRES a proper probability vector at every visited `(s, x)` and rejects negative, non-finite, or non-unit-sum weights at the sampling boundary, since missing mass has no path-wise interpretation). Action-dependent weights force the Brent fallback (the workspace drops `dfecs`); state-only weights keep the FOC path.
- Exceptions by design: LQA ignores weights (certainty-equivalent around the approximation point) but REJECTS problems carrying an injected kernel (`ArgumentError`: it linearizes `cdp.g`, which an injected kernel replaces); `simulate!` keeps the precomputed-draws path for fixed weights and draws sequentially per step via `_draw_branch_index` for callable weights.
